...
int main()
{
	...
	char buffer[MAX_BUFFER + 1] = "SOURAV ##\n";
	...
	for( ; ; )
	{
		...
		for(i = 0; i< 3; i++)
		{
			// changing 9th character after # in the message buffer

			buffer[9] = i + '1';
			sctp_sendmsg(cfd, (void *)buffer, (size_t)strlen(buffer), NULL, 0, 0, 0, i /*stream*/, 0, 0);
			printf("Sent: %s\n", buffer);
		}
